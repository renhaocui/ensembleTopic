...
LLDALimitSet = [0.0, 0.1, 0.3, 0.5]
MaxEntLimitSet = [0.0, 0.1, 0.3, 0.5]
priorityLimitSet = [2.0, 0.5, 0.7, 0.9]

for brand in brandList:
    print brand
    for limit in priorityLimitSet:
        accuracy = 0.0
        for j in range(fold):
            # split the data
            LLDAData = {}
            ...
            LLDAoutputFile = open('LLDAoutput//' + brand + '.' + str(j)+',0.1', 'r')
            trueLabelFile = open('trueLabel//'+brand+'.'+str(j), 'r')
            maxEntFile = open('MaxEntoutput//' + brand + '.' + str(j)+',0.1', 'r')
                    ...
                    if len(maxEntTopics) == 0:
                        ...
                        for topic in uniqueTopics:
                            ...
                            if topic in maxEntTopics:
                                predTopics[topic] += maxEntTopics[topic]
                            if topic in LLDATopics:
                                predTopics[topic] += LLDATopics[topic]
                ...
                if len(LLDATopics) == 0:
                    ...
                    for (topic, prob) in LLDATopics.items():
                        if topic in mappingTopics:
                            predTopics[topic] = prob + mappingTopics[topic]
                        else:
                ...
                if len(maxEntTopics) == 0:
                    ...
                    for topic in uniqueTopics:
                        ...
                        if topic in maxEntTopics:
                            predTopics[topic] += maxEntTopics[topic]
                        if topic in mappingTopics:
                            predTopics[topic] += mappingTopics[topic]
                    predTopic = max(predTopics.iteritems(), key=operator.itemgetter(1))[0]
                '''
                # all
